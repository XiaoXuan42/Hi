...
export class FileTree {
    ...
    private add_file(dirnode: Dir, abspath: string, is_private: boolean): File {
        let new_file: File = this.file_generator.generate_file(
            abspath,
            dirnode.url,
            is_private
        )
    ...
    private private_encrypt(plaintext: string): string {
        ...
        const context = {
            ...
            private_scripts: get_private_scripts(
                this.config.get_project_name()
            ),
        }
        return nunjucks.renderString(
            this.config.get_template("private.jinja"),
            context
        )
    }
    ...
    private get_relpath(abspath: string): string {
        const relpath = path
            .relative(this.config.project_root_dir, abspath)
            .replace(path.sep, "/")
        if (relpath[0] !== "/") {
            return "/" + relpath
        }
    ...
    private _visit_url(
        fnode: FNode,
        callback: (fnode: File) => any,
        response: (fnode: FNode, res: any) => any
    ) {
    ...
    public get_result_content(
        url: string,
        converter: (file: File) => string
    ): string | undefined {
    ...
    public search_by_url_pattern(url_pattern: string): File[] {
        ...
        let queue: [{ node: FNode; index: number }] = [
            { node: this.fnode_root, index: 0 },
        ]
